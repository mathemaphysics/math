...
int main( int argc, char **argv )
{
	...
	{
		nlbase[i] = 6;
		lbase[i] = (int*) malloc( 32 * sizeof(int) );
		lbase[i][0] = 0;
		lbase[i][1] = 1;
		lbase[i][2] = 2;
		lbase[i][3] = 3;
	...
	/* Poisson problem starts here */
	double *b = (double*) malloc( mdim * sizeof(double) );
	double *xx = (double*) malloc( mdim * sizeof(double) );
	for(i=0;i<dim;i++)
		for(j=0;j<dim;j++)
			if( j == i )
				nqbox[i*dim+j] = 1.0;
	...
	{
		for(j=0;j<nlbase[ltg[i]];j++)
		{
			global_polynomial_vector( lbase[ltg[i]][j], dim, expl );
			for(k=0;k<dim;k++)
				index[k] = 0, size[k] = quadn - 1;
			sum = 0.0;
			do
			{
				sphere_gauss_point( dim, pt.pts + ltg[i] * dim, pt.dlt[ltg[i]], nqbox, index, qpts, qwts, qx, &qw );
	...
	scg( mdim, ia, ja, A, b, xx, 10000, 1e-9, 1, &ret );

	/* Eigenvalue problem starts here */
				...
				for(ii=0;ii<nlbase[ltg[i]];ii++)
				{
					...
					if( y < pt.dlt[ltg[i]] )
					{
						if( lbase[ltg[i]][ii] >= 0 )
						{
							global_polynomial_vector( lbase[ltg[i]][ii], dim, expl );
							if( b_use_singular )
								sum += xx[pp] * punity_eval_local_poly_delta( &pt, ltg[i], expl, qx, i_sing_order );
							else
								sum += xx[pp] * punity_eval_local_poly( &pt, ltg[i], expl, qx );
						}
						else
						{
							/* Deal with plotting of localized extra RBFs */
							for(j=0;j<dim;j++)
								ctr[j] = qx[j] - pt.pts[ltg[i]*dim+j];
							sum  += xx[pp] * rbf[-1-lbase[ltg[i]][ii]]( dim, 0, pt.dlt[ltg[i]], ctr ) * punity_evaluate( &pt, ltg[i], qx );
						}
					}
					pp++;
				}
