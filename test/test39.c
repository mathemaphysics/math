...
int main( int argc, char **argv )
{
	...
	int dim,deg,rdn,npts,nbp,mdim,*drv,*expl,*expr,*ltg,*nlbase,**lbase;
	double *pts,*dlt;
	punity_t pt;
						...
						if( sphere_intersection( dim, pt.pts + ltg[i] * dim, pt.dlt[ltg[i]], pt.pts + ltg[j] * dim, pt.dlt[ltg[j]], ctr, &rad, qbox ) == 1 )
						{
							...
							do
							{
								...
								if( ret == 1 )
								{
									...
									{
										if( b_use_singular )
											x = punity_eval_local_poly_delta( &pt, ltg[i], expl, qx, i_sing_order )
												* punity_eval_local_poly_delta( &pt, ltg[j], expr, qx, i_sing_order );
										else
											x = punity_eval_local_poly( &pt, ltg[i], expl, qx )
                                                                                                * punity_eval_local_poly( &pt, ltg[j], expr, qx );
										sum += qw * x;
									}
									if( !b_load_stiff_mat )
									{
										...
										for(k=0;k<dim;k++)
										{
											...
											if( b_use_singular )
												y += punity_term_deriv_local_poly_delta( &pt, ltg[i], expl, drv, qx, i_sing_order, NULL, 0 )
													* punity_term_deriv_local_poly_delta( &pt, ltg[j], expr, drv, qx, i_sing_order, NULL, 0 );
											else
												y += punity_term_deriv_local_poly( &pt, ltg[i], expl, drv, qx, NULL, 0 )
                                                                                                        * punity_term_deriv_local_poly( &pt, ltg[j], expr, drv, qx, NULL, 0 );
										}
										tum += qw * y;
									}
								}
