...
int main( int argc, char **argv )
{
	...
	if( !b_boundary_proj && i_max_lanczos_steps > npts - nbp )
	{
	...
	if( b_boundary_proj && b_save_proj_mat )
	{
		...
		for(i=0;i<npts-nbp;i++)
		{
			...
			project( npts, W + i * npts, nbp, V );
			project( npts, W + i * npts, i, W );
			normalize( npts, W + i * npts );
		}
		msave( s_proj_fname, npts - nbp, npts, W, 1 );
		fprintf( stderr, " * Saved projection matrix to file \"%s\"\n\n", s_proj_fname );
		free( W );
	}
							...
							if( ret == 1 )
							{
								...
								if( !b_load_stiff_mat )
								{
									...
									for(k=0;k<dim;k++)
									{
										...
										y += punity_term_deriv_evaluate( &pt, i, drv, qx, NULL, 0 )
											* punity_term_deriv_evaluate( &pt, j, drv, qx, NULL, 0 );
									}
									tum += qw * y;
								}
							}
	...
	if( b_boundary_proj )
		eig = (double*) malloc( neig * npts * sizeof(double) );
	else
	...
	if( b_boundary_proj )
		csgsilanczos( npts, neig, ia, ja, A, ib, jb, B, alpha, beta, omega, NULL, nbp, V, 1e-9, 10000, 0, &ret ), mo = neig;
	else
	...
	if( b_boundary_proj )
	{
		...
		for(i=0;i<neig;i++)
		{
			nrandv( npts, eig + i * npts );
			sgsinvi( npts, ia, ja, A, ib, jb, B, eig + i * npts, ev[2*i+0], 1e-9, 500, 0 );
			fprintf( stderr, " ----> %5d EV = %15.7f: ", i, ev[2*i+0] );
			for(j=0;j<NUM_EIG_DIMS_TO_PRINT;j++)
				fprintf( stderr, "%15.7f", eig[i*npts+j] );
			fprintf( stderr, "  ...\n" );
		}
		fprintf( stderr, "\n" );
	}
	else
	{
		...
		for(i=0;i<neig;i++)
		{
			...
			sgsinvi( npts - nbp, ia, ja, A, ib, jb, B, eig + i * ( npts - nbp ), 1.0 / ev[2*i+0], 1e-9, 500, 0 );
	...
	if( b_out_eig_range_set )
	{
		...
		do
		{
			...
			for(n=0;n<nfr;n++)
			{
				...
				{
					sum = 0.0;
					for(i=0;i<(b_boundary_proj?npts:npts-nbp);i++)
					{
						y = 0.0;
						for(j=0;j<dim;j++)
							y += pow( qx[j] - pt.pts[(b_boundary_proj?i:ltg[i])*dim+j], 2.0 );
						y = sqrt( y );
						if( y < pt.dlt[(b_boundary_proj?i:ltg[i])] )
							sum += eig[m*(npts-nbp)+i] * punity_evaluate_delta( &pt, (b_boundary_proj?i:ltg[i]), qx, i_sing_order );
					}
