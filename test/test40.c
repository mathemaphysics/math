...
double exp_func( int dim_in, int drv_in, double dlt_in, double *x_in )
{
	int i,n=1;
	double r,rho;
	double tot = pow( -0.5, (double) drv_in );
...
int main( int argc, char **argv )
{
	...
	double lx1[2] = { 0.2, 0.2 };
	double lx2[2] = { 0.4, 0.4 };
	double lr1 = sqrt(2.0) * 0.1 * 1.1;
	double lr2 = sqrt(2.0) * 0.1 * 1.1;
	int drv[2] = { 0, 0 };
	int pdx[2] = { 0, 1 };
	int idx1 = 10, idx2 = 11;
	fprintf( stderr, "%15.7f %15.7f and %15.7f %15.7f\n", pts[idx1*dim+0], pts[idx1*dim+1], pts[idx2*dim+0], pts[idx2*dim+1] );
	FILE *fp = fopen( "circles.out", "w" );
	int astep = 100;
	for(i=0;i<astep;i++)
	{
		qx[0] = pts[idx1*dim+0] + dlt[idx1] * cos( 2.0 * M_PI / (double) ( astep - 1 ) * (double) i );
		qx[1] = pts[idx1*dim+1] + dlt[idx1] * sin( 2.0 * M_PI / (double) ( astep - 1 ) * (double) i );
		fprintf( fp, "%15.7f%15.7f\n", qx[0], qx[1] );
	}
	for(i=0;i<astep;i++)
	{
		qx[0] = pts[idx2*dim+0] + dlt[idx2] * cos( 2.0 * M_PI / (double) ( astep - 1 ) * (double) i );
		qx[1] = pts[idx2*dim+1] + dlt[idx2] * sin( 2.0 * M_PI / (double) ( astep - 1 ) * (double) i );
		fprintf( fp, "%15.7f%15.7f\n", qx[0], qx[1] );
	}
	fclose( fp );
	fp = fopen( "intersect.out", "w" );
	if( sphere_intersection( dim, pt.pts + idx1 * dim, pt.dlt[idx1], pt.pts + idx2 * dim, pt.dlt[idx2], ctr, &rad, qbox ) == 1 )
	{
		fprintf( stderr, " * Point intersect!\n\n" );
		for(i=0;i<dim;i++)
		{
			sum = 0.0;
			for(j=0;j<dim;j++)
		...
		do
		{
			lens_gauss_point( dim, pt.pts + idx1 * dim, pt.dlt[idx1],  pt.pts + idx2 * dim, pt.dlt[idx2], rad, nqbox, index, qpts, qwts, qx, &qw );
			domain_indicator( qx, &dm, &ret );
			if( ret == 1 )
			{
				fprintf( fp, "%15.7f%15.7f\n", qx[0], qx[1] );
				x = punity_term_deriv_local_poly( &pt, idx1, pdx, drv, qx, NULL, 0 )
					* punity_term_deriv_local_poly( &pt, idx2, pdx, drv, qx, NULL, 0 );
				sum += qw * x;
			}
		}
		while( arraynext( (long) dim, size, index ) != -1 );
		fprintf( stderr, " ---> Radius of %d is %15.7f and radius of %d is %15.7f\n", idx1, pt.dlt[idx1], idx2, pt.dlt[idx2] );
		fprintf( stderr, " ---> Total quadrature sum = %15.7f\n", sum );
		fprintf( stderr, "\n" );
	}
	fclose( fp );
	...
	do
	{
		...
		/////////////////////////////////////////////////////////////
		//sum = punity_term_deriv_local_radial( &pt, 55, exp_func, drv, sx, NULL, 0 );
		for(j=0;j<dim;j++)
			qx[j] = sx[j] - 2.5;
		sum = cubic_window( 2, 1.0, qx ) * exp_func( 2, 0, 1.0, qx );
		//sum = exp_func( dim, 0, 1.0, qx );
