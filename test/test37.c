...
int main( int argc, char **argv )
{
	...
	/* Output points with radii */
	fprintf( stderr, " * Outputting points and radii for plotting to \"circles.out\"\n\n" );
	fp = fopen( "circles.out", "w" );
	for(i=0;i<npts;i++)
	{
		for(j=0;j<dim;j++)
			fprintf( fp, "%15.7f", pts[i*dim+j] );
		fprintf( fp, "%15.7f\n", dlt[i] );
	}
	fclose( fp );
	...
	do
	{
		...
		for(i=0;i<dim;i++)
			fprintf( stdout, "%15.10f", qx[i] );

		/* Evaluate localize monomial terms */
		int xp[2] = { 1, 2 }; int dr[2] = { 0, 0 };
		//sum = punity_eval_local_poly( &pt, 15, xp, qx );
		sum = punity_term_deriv_local_poly( &pt, 4, xp, dr, qx, NULL, 0 );

		/* Ouptut the values consecutively */
		fprintf( stdout, "%17.12f", sum );
