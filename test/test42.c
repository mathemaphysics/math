...
int main( int argc, char **argv )
{
	...
	/* Choose index of function to plot */
	int idx = 19;
	int en = 2, el = 0;
	double a0 = 0.09; /* Angstroms */
	...
	do
	{
		...
		rr = 0.0;
		for(i=0;i<dim;i++)
			rr += pow( qx[i] - pts[idx*dim+i], 2.0 );
		rr = sqrt( rr );
		sum = radial_wave_evaluate( en, el, rr, a0 ) * punity_evaluate( &pt, idx, qx );
