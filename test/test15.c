...
int main( int argc, char **argv )
{
	...
	for(i=0;i<npts;i++)
		gqw[i] = 1.0; //0.9 * 4.0 * M_PI * dlt[i] * dlt[i];
	...
	/* Check for partition of unity */
        ...
        for(i=0;i<npts;i++)
        {
                xx = rkp_term_evaluate_node( &rk, i, m );
                sum += xx;
        }
		...
		for(i=0;i<pgx;i++)
       		{
               		for(j=0;j<pgy;j++)
               		{
                       		xx = rkp_term_evaluate( &rk, m, ppts + i * pgy * dim + j * dim );
	...
	else
	{
		...
		for(i=0;i<npts;i++)
		{
			xx = rkp_term_evaluate( &rk, m, pts + i * dim );
	...
	for(i=0;i<npts;i++)
	{
		...
		{
			for(k=0;k<dim;k++)
				amat[i*npts+j] -= 0.5 * gqw[j] * rkp_wavelet_term_evaluate_node( &rk, j, ord[k], i );
			amat[i*npts+j] += gqw[j] * rkp_term_evaluate_node( &rk, j, i ) * coulomb_potential( dim, pts + i * dim, nnuc, nuc );
			bmat[i*npts+j] = gqw[j] * rkp_term_evaluate_node( &rk, j, i );
		}
	}
